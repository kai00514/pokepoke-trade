-- ===== 既存のトレード通知生成関数 (trigger_trade_comment_notification が呼び出す関数) の修正 =====
-- 関数名は既存のものに合わせる必要があります。ここでは create_trade_notification_for_comment と仮定します。
-- もし関数名が異なる場合は、実際の関数名に置き換えてください。
CREATE OR REPLACE FUNCTION create_trade_notification_for_comment(
  p_trade_post_id UUID,
  p_commenter_user_id TEXT,    -- trade_comments.user_id が TEXT 型のため
  p_commenter_name TEXT,
  p_comment_content TEXT      -- 通知内容にコメントの一部を含める場合など
) RETURNS VOID AS $$
DECLARE
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'Error in create_trade_notification_for_comment for trade_post_id %: %', p_trade_post_id, SQLERRM;
END;
$$ LANGUAGE plpgsql;


-- ===== 既存のデッキ通知生成関数 (trigger_deck_comment_notification が呼び出す関数) の修正 =====
-- 関数名は既存のものに合わせる必要があります。ここでは create_deck_notification_for_comment と仮定します。
CREATE OR REPLACE FUNCTION create_deck_notification_for_comment(
  p_deck_id UUID,
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'Error in create_deck_notification_for_comment for deck_id %: %', p_deck_id, SQLERRM;
END;
$$ LANGUAGE plpgsql;
