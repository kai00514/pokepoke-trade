...
CREATE POLICY "Authenticated users can insert trade posts" ON trade_posts
    FOR INSERT WITH CHECK (
        ...
        (auth.uid() IS NOT NULL AND owner_id::text = auth.uid()::text) OR 
    ...
    FOR INSERT WITH CHECK (
        EXISTS (
            ...
            WHERE id = post_id AND (
                (owner_id IS NOT NULL AND auth.uid() IS NOT NULL AND owner_id::text = auth.uid()::text) OR
                (owner_id IS NULL AND guest_name IS NOT NULL)
            )
        )
    );
...
CREATE POLICY "Users can update their wanted cards" ON trade_post_wanted_cards
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM trade_posts 
            WHERE id = post_id AND auth.uid() IS NOT NULL AND owner_id::text = auth.uid()::text
        )
    );
...
CREATE POLICY "Authenticated users can insert comments" ON trade_comments
    FOR INSERT WITH CHECK (
        ...
        (auth.uid() IS NOT NULL AND user_id IS NOT NULL AND user_id::text = auth.uid()::text) OR 
        (user_id IS NULL AND guest_name IS NOT NULL)
    );

CREATE POLICY "Users can update their own comments" ON trade_comments
    FOR UPDATE USING (
        auth.uid() IS NOT NULL AND user_id IS NOT NULL AND user_id::text = auth.uid()::text
    );
...
SELECT 'Testing guest post insertion...' as test_status;
