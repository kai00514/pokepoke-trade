...
DECLARE
  ...
  commenters RECORD;
BEGIN
  RAISE NOTICE 'create_trade_comment_notification: Start for post_id=%s, commenter_user_id=%s', p_post_id, p_commenter_user_id;
  ...
  RAISE NOTICE 'create_trade_comment_notification: Post owner_id=%s, post_title=%s', post_owner_id, post_title;
  ...
  IF post_owner_id IS NOT NULL AND post_owner_id != p_commenter_user_id THEN
    BEGIN
      ...
      RAISE NOTICE 'create_trade_comment_notification: Notification inserted for post owner %s', post_owner_id;
    EXCEPTION
      WHEN OTHERS THEN
        RAISE WARNING 'create_trade_comment_notification: Error inserting notification for post owner: %s', SQLERRM;
    END;
  ELSE
  ...
  LOOP
    BEGIN
      ...
      RAISE NOTICE 'create_trade_comment_notification: Notification inserted for other commenter %s', commenters.user_id;
    EXCEPTION
      WHEN OTHERS THEN
        RAISE WARNING 'create_trade_comment_notification: Error inserting notification for other commenter %s: %s', commenters.user_id, SQLERRM;
    END;
  END LOOP;
...
DECLARE
  ...
  commenters RECORD;
BEGIN
  RAISE NOTICE 'create_deck_comment_notification: Start for deck_id=%s, commenter_user_id=%s', p_deck_id, p_commenter_user_id;
  ...
  RAISE NOTICE 'create_deck_comment_notification: Deck owner_id=%s, deck_name=%s', deck_owner_id, deck_name;
  ...
  IF deck_owner_id IS NOT NULL AND deck_owner_id != p_commenter_user_id THEN
    BEGIN
      ...
      RAISE NOTICE 'create_deck_comment_notification: Notification inserted for deck owner %s', deck_owner_id;
    EXCEPTION
      WHEN OTHERS THEN
        RAISE WARNING 'create_deck_comment_notification: Error inserting notification for deck owner: %s', SQLERRM;
    END;
  ELSE
  ...
  LOOP
    BEGIN
      ...
      RAISE NOTICE 'create_deck_comment_notification: Notification inserted for other commenter %s', commenters.user_id;
    EXCEPTION
      WHEN OTHERS THEN
        RAISE WARNING 'create_deck_comment_notification: Error inserting notification for other commenter %s: %s', commenters.user_id, SQLERRM;
    END;
  END LOOP;
...
AS $$
BEGIN
  RAISE NOTICE 'trigger_trade_comment_notification: Trigger fired for NEW.user_id=%s', NEW.user_id;
  -- ゲストユーザーのコメントは通知しない
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'trigger_trade_comment_notification: Error in trigger: %s', SQLERRM;
...
AS $$
BEGIN
  RAISE NOTICE 'trigger_deck_comment_notification: Trigger fired for NEW.user_id=%s', NEW.user_id;
  -- ゲストユーザーのコメントは通知しない
  IF NEW.user_id IS NOT NULL THEN
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'trigger_deck_comment_notification: Error in trigger: %s', SQLERRM;
    RETURN NEW; -- エラーが発生しても挿入処理は続行
END;
$$;
