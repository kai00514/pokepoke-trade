-- ===== 1. トレード通知生成関数のリネーム =====
-- 既存の関数を削除し、正しい名前で再作成します。
-- 引数の型と順序は、trigger_trade_comment_notification が呼び出すものに合わせます。
-- (p_post_id UUID, p_user_id TEXT, p_user_name TEXT)
DROP FUNCTION IF EXISTS create_trade_notification_for_comment(UUID, TEXT, TEXT, TEXT);
...
-- ===== 2. デッキ通知生成関数のリネーム =====
-- 既存の関数を削除し、正しい名前で再作成します。
-- 引数の型と順序は、trigger_deck_comment_notification が呼び出すものに合わせます。
-- (p_deck_id UUID, p_user_id UUID, p_user_name TEXT, p_comment_type TEXT)
DROP FUNCTION IF EXISTS create_deck_notification_for_comment(UUID, UUID, TEXT, TEXT, TEXT);
