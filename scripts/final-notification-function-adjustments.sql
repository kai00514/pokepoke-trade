-- ===== 1. トレード通知生成関数の修正 =====
-- 既存のトリガーが呼び出す関数名 `create_trade_comment_notification` に合わせ、引数を調整します。
-- `p_comment_content` はトリガーから渡されないため、関数内では使用しないか、デフォルト値を持つようにします。
...
-- ===== 2. デッキ通知生成関数の修正 =====
-- 既存のトリガーが呼び出す関数名 `create_deck_comment_notification` に合わせ、引数を調整します。
