...
DROP FUNCTION IF EXISTS public.create_trade_comment_notification(UUID, TEXT, TEXT); -- 古いTEXT型引数の関数を削除
DROP FUNCTION IF EXISTS public.create_trade_comment_notification(UUID, UUID, TEXT); -- 新しいUUID型引数の関数を削除 (念のため)
DROP FUNCTION IF EXISTS public.create_deck_comment_notification(UUID, UUID, TEXT, TEXT); -- 古いTEXT型引数の関数を削除
DROP FUNCTION IF EXISTS public.create_deck_comment_notification(UUID, UUID, TEXT, TEXT); -- 新しいUUID型引数の関数を削除 (念のため)
...
CREATE OR REPLACE FUNCTION public.create_trade_comment_notification(
  p_post_id UUID,
  p_commenter_user_id UUID,  -- TEXTからUUIDに変更
...
BEGIN
  ...
  IF post_owner_id IS NOT NULL AND post_owner_id IS DISTINCT FROM p_commenter_user_id::TEXT THEN -- UUIDとTEXTの比較のためキャスト
  ...
  FOR commenters IN
    ...
    WHERE post_id = p_post_id 
      ...
      AND user_id IS DISTINCT FROM p_commenter_user_id::TEXT  -- コメント投稿者自身は除く (UUIDとTEXTの比較のためキャスト)
...
BEGIN
  ...
  IF p_comment_type = 'deck' AND deck_owner_id IS NOT NULL AND deck_owner_id IS DISTINCT FROM p_commenter_user_id::TEXT THEN -- UUIDとTEXTの比較のためキャスト
